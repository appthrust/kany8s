...
Both charts support `aws.mode={staticSecret,irsa,podIdentity}` and a
Bitnami-style `image.{registry,repository,tag,digest}` split with a
`global.imageRegistry` override for mirrored / air-gapped environments. See
the per-chart READMEs at `charts/eks-kubeconfig-rotator/README.md` and
