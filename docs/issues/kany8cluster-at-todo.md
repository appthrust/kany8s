...
- [ ] RGD 名を `demo-infra.kro.run` に確定する
...
- [ ] hack script が `KIND_CLUSTER_NAME` を受け取れるようにする
...
- [ ] hack script が `KRO_RGD_MANIFEST` を override できるようにする
...
- [ ] `sed -e 's/__CLUSTER_NAME__/demo-cluster/g' -e 's/__NAMESPACE__/default/g' -e 's/__RGD_NAME__/demo-infra.kro.run/g' test/acceptance_test/manifests/kro/kany8scluster.yaml.tpl > /tmp/kany8scluster.yaml` を実行する
...
- [ ] `repo_root` を `hack/acceptance-test-kro-reflection.sh` と同じ方式で解決する
- [ ] `cd "${repo_root}"` を追加する
...
- [ ] `cleanup()` を追加する（先頭で `restore_kustomization` を呼ぶ）
- [ ] `cleanup()` に `CLEANUP=true` の場合の `kind delete cluster --name ... --kubeconfig ...` を追加する
- [ ] `cleanup()` に `CLEANUP=false` の場合の “keep” メッセージを追加する
...
- [ ] `k -n "${NAMESPACE}" wait --for=condition=Ready --timeout=240s "kany8scluster/${CLUSTER_NAME}"` を追加する
...
- [ ] `chmod +x hack/acceptance-test-kro-infra-reflection.sh` を実行する
