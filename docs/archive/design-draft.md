...
### 4.3 この repo にある "入口"（CAPI / Kany8s）

Kany8s の設計上の主入口は CAPI です。本 repo では主に次の 2 つの入口を扱います。

1) managed control plane（kro mode; `Kany8sControlPlane`）

- 入口: `controlplane.cluster.x-k8s.io/v1alpha1` `Kany8sControlPlane`
- `spec.resourceGraphDefinitionRef` で RGD を選択し、Kany8s が 1:1 で kro instance を作成/更新する
- Kany8s が読むのは kro instance の正規化 status（`status.ready/endpoint/reason/message`）のみ（契約は `docs/rgd-contract.md`）
- 例/検証: `hack/acceptance-test-kro-reflection.sh` / `examples/capi/cluster.yaml` / `test/acceptance_test/manifests/kro/rgd.yaml`

2) self-managed（CAPD + kubeadm; `Kany8sKubeadmControlPlane`）

- 入口: CAPI `Cluster` + CAPD `DockerCluster` + `Kany8sKubeadmControlPlane`
- 目的: kubeadm/CAPD の control plane provider contract を満たし、実際に到達可能な workload cluster を作る
- kro は使わない（kro を control plane の "具象化エンジン" として使う設計とは別軸）
- 例/検証: `hack/acceptance-test-capd-kubeadm.sh` / `test/acceptance_test/manifests/self-managed-docker/cluster.yaml.tpl`
- 注: manual 用の素の例は `examples/self-managed-docker/cluster.yaml`（受け入れではテンプレを render して適用する）
...
- `status.kubeconfigSecretRef: object`
  - `name: string`
  - `namespace: string`（optional; 省略時は kro instance の namespace とみなす）
  - **意味**: provider-agnostic に kubeconfig を取得するための参照（SecretRef）。
  - **契約**: 参照先 Secret は kubeconfig を `data.value` に格納する。
...
- `spec.kroSpec: object`（optional）
  ...
  - NOTE: `spec.kroSpec` must be a JSON object because the controller injects `spec.version` into the kro instance `.spec`.
