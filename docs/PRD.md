...
- 設計判断 (採用/非採用/トレードオフ) は ADR として `docs/adr/` に集約します。
- 詳細設計の補足や調査ログは `docs/reports/` / `docs/archive/` に置きます。
...
- 位置付け: Cluster API provider suite
  - `Kany8sControlPlane` (managed control plane / kro; planned: facade for multiple backends)
...
- 観測性は正規化 status に集約: Kany8s とユーザーが provider-specific な子リソースを覗かずに状況判断できるよう、kro instance の正規化 status (`ready/endpoint/reason/message/kubeconfigSecretRef`) を parent RGD でも可能な限り投影する。
  - planned: self-managed/external backend も同じ正規化 status contract を採用し、facade が provider-specific な shape を読まない境界を維持する。
...
- infrastructure
  - `Kany8sCluster` は CAPI v1beta2 InfrastructureCluster contract（`status.initialization.provisioned`）を満たす
  - default: stub mode（`spec.resourceGraphDefinitionRef` なし）では `provisioned=true` を立てて unblock する
  - optional: kro mode（`spec.resourceGraphDefinitionRef` あり）では、kro instance を 1:1 で作成/更新し、instance `status.ready/reason/message` を `provisioned`/Conditions に反映する
    - 付随: cluster identity を instance `.spec.clusterName/.spec.clusterNamespace` に注入する（OwnerReference が必要な provider 向けに `clusterUID` 注入も opt-in で対応）
...
- self-managed ControlPlane backend の拡張 (facade)
  - `Kany8sControlPlane` を "唯一の CAPI-facing ControlPlane" としつつ、kro/kubeadm/external backend を選択できるようにする
  - backend は正規化 status contract (`ready/endpoint/reason/message/kubeconfigSecretRef` + 推奨: `terminal/observedGeneration`) を満たす
  - 制約: backend は namespaced 前提、backend 選択は one-of、作成後の backend 種別変更は禁止（切替は置換として扱う）
  - 参照: `docs/adr/0011-extensible-controlplane-backends.md`
...
- Planned: `Kany8sControlPlane` の backend 選択入力面
  - 目的: `Cluster.spec.controlPlaneRef` は常に `Kany8sControlPlane` を指し、backend は facade が 1:1 で作成/所有する
  - one-of で backend を選択 (kro / kubeadm delegate / external backend)
  - external backend の `.spec` は arbitrary JSON を許容しつつ、`spec.version` と cluster identity を facade が注入/上書きする
...
- Planned: `Kany8sControlPlane` facade reconcile (external/kubeadm delegate)
  - backend object を 1:1 で作成/更新し、backend status 正規化 contract のみを読んで facade の contract を満たす
  - backend GVK を動的 watch し、status 変化で reconcile を駆動できる
  - RBAC は backend ごとに install-time で拡張し、`*.*` wildcard は採用しない
...
- Must: `Kany8sCluster` reconcile
  - stub mode: `spec.resourceGraphDefinitionRef` が無い場合は `status.initialization.provisioned=true` を設定し、`Ready=True` を立てる
  - kro mode: `spec.resourceGraphDefinitionRef` がある場合は:
    - RGD を取得し、生成される instance GVK を解決できる
    - kro instance を 1:1 で作成/更新できる (OwnerReference 付与)
...
推奨 (planned / non-breaking):

- `status.terminal: boolean` (backend が unrecoverable failure を宣言)
- `status.observedGeneration: int64` (backend が最新 desired state を観測済みか)
...
- `status.kubeconfigSecretRef` による Secret 参照は同一 namespace をデフォルトとし、cross-namespace は明示 opt-in (RBAC + allowlist 推奨) とする
...
- ADR (設計判断): `docs/adr/README.md`
  - provider-agnostic / kro 委譲: `docs/adr/0001-provider-agnostic-kany8s-via-kro.md`
  - RGD status contract: `docs/adr/0002-normalized-rgd-instance-status-contract.md`
  - spec 注入/1:1 instance 管理: `docs/adr/0003-kro-instance-lifecycle-and-spec-injection.md`
  - kubeconfig Secret 方針: `docs/adr/0004-kubeconfig-secret-strategy.md`
  - Topology version / template group: `docs/adr/0006-topology-version-and-template-api-groups.md`
  - dynamic GVK RBAC tradeoff: `docs/adr/0007-dynamic-gvk-rbac-tradeoffs.md`
  - infra outputs 方針(Approach A): `docs/adr/0008-infra-outputs-policy-parent-rgd-approach-a.md`

- RGD contract / authoring: `docs/reference/rgd-contract.md`, `docs/reference/rgd-guidelines.md`
- kro pitfalls (kind + kro v0.7.1): `docs/reference/kro-v0.7.1-kind-notes.md`
- 実装タスク: `docs/TODO.md`
- Codebase map: `docs/reference/codebase.md`
- Reviews/Reports (historical): `docs/reviews/`, `docs/reports/`
- E2E/Acceptance guides: `docs/guides/e2e-guide.md`, `docs/guides/e2e-and-acceptance-test.md`
- Runbooks: `docs/runbooks/`

## 14. Appendix (Historical)

- Infrastructure kro mode の実装チェックリスト（履歴）: `docs/archive/prd-infra-kany8scluster-kro-mode-checklist.md`
