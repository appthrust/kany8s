...
Kany8s は Cluster API (CAPI) と kro (ResourceGraphDefinition / RGD) を組み合わせ、managed Kubernetes control plane (EKS/GKE/AKS 等) だけでなく、kubeadm による self-managed control plane も含めて "kubernetes-native" に作成・運用するための Cluster API provider suite です。

プロジェクト目標: (前提: 管理クラスタに CAPI core + CABPK + infrastructure provider を導入した上で) Kany8s の定義（Kany8s CRD/controller + kro RGD）だけで、少なくとも CAPD(docker) 上に "実際に動作する" Kubernetes クラスタを作成できることを保証する（CAPI の `RemoteConnectionProbe=True` / `Cluster Available=True` まで到達でき、kubeconfig で接続できる）。
...
### MVP (現状: managed control plane の最小実装)
...
- kubeconfig Secret は `kro instance.status.kubeconfigSecretRef` が提示される場合に `<cluster>-kubeconfig` を整形/作成できる（copy & normalize）。
- 制限: kubeadm ベースの self-managed cluster を成立させる Machine/証明書/kubeconfig（自動生成）までは未対応。
...
現状の不足（CAPD + kubeadm で "実動作クラスタ" に至るためのギャップ）:

- ControlPlane Machine の作成/管理が無い（`Machine`/`DockerMachine*`/`KubeadmConfig` の生成・スケール・アップグレード）
- kubeadm 用の証明書管理が無い（Cluster CA/front-proxy/SA key 等）
- `<cluster>-kubeconfig` Secret の生成が無い（ControlPlaneRef を使う Cluster では CAPI 側が生成しない）
- endpoint の整合（Kany8sControlPlane が infra endpoint を上書きし得る）
...
- Worker(MachineDeployment/MachinePool) の自動作成・管理（当面）。ただし self-managed 目標では control plane の Machine 管理は必須。
...
- kubeconfig Secret 管理 (CAPI contract 上必須)
  - 現状: kro instance が `status.kubeconfigSecretRef` を出せる場合は Kany8s が `<cluster>-kubeconfig` を整形/作成できる
  - 追加目標: kubeadm mode では Kany8s 自身が kubeconfig を生成し、RemoteConnectionProbe を成立させる
...
- Must (プロジェクト目標): kubeadm control plane provisioning (KubeadmControlPlane 相当)
...
- Should: `status.kubeconfigSecretRef` (name/namespace). managed control plane など "外部生成 kubeconfig" を使う場合の入力契約。
...
## M2b: CAPD(docker) + kubeadm で self-managed control plane (KubeadmControlPlane 相当)

MVP の位置づけ:

- 目的: CAPD + kubeadm で "実 API server を持つ" workload cluster を作り、`RemoteConnectionProbe=True` / `Cluster Available=True` に到達する。
- 前提: 管理クラスタに CAPI core + CABPK + CAPD がインストール済み。
...
- [x] self-managed の責務/境界を設計として固定する
  - 既定:
    ...
    - endpoint source of truth: infrastructure provider の `spec.controlPlaneEndpoint` を優先（Kany8s が推測値で上書きしない）
    - `<cluster>-kubeconfig` Secret: Kany8s が生成・維持（Cluster API contract を満たす）
    - 証明書: Cluster API `util/secret` の naming/format に合わせて生成・管理し、CABPK の bootstrap で利用する
...
- [x] `Kany8sKubeadmControlPlaneSpec` を定義する（KCP 互換の最小）
  - 追加/仕様(目安):
    ...
    - `spec.machineTemplate.infrastructureRef` (required; `clusterv1.ContractVersionedObjectReference`)
    - `spec.kubeadmConfigSpec` (optional; `bootstrapv1.KubeadmConfigSpec`)
    - `spec.controlPlaneEndpoint` (optional; `clusterv1.APIEndpoint`; controller が設定)
...
- [x] `Kany8sKubeadmControlPlaneStatus` を定義する（CAPI contract の最小）
  - 追加:
    ...
    - `status.initialization.controlPlaneInitialized`
    - `status.conditions` + `GetConditions/SetConditions`
...
- [x] Cluster CA Secret から admin kubeconfig を生成し `<cluster>-kubeconfig` を作成/更新する
  ...
  - Touch: `internal/controller/kany8skubeadmcontrolplane_controller.go`
  - DoD:
    - Secret 名/labels/type/data.key が contract と一致
...
- [ ] control plane `Machine` を作成し、infra/bootstrap を参照させる
  - 仕様(目安):
    ...
    - `spec.infrastructureRef` -> infraMachine
    - control plane label を付与（例: `cluster.x-k8s.io/control-plane`）
  - Touch: `internal/controller/kany8skubeadmcontrolplane_controller.go`
...
- [ ] fake client unit test で self-managed controller の最小フローを固定する
  ...
  - シナリオ例:
    ...
    - endpoint 設定後 -> cert secrets / kubeconfig secret / Machine / KubeadmConfig / infraMachine が作成される
...
- [ ] self-managed acceptance test を追加する（`RemoteConnectionProbe=True` / `Cluster Available=True`）
  ...
  - DoD:
    ...
    - `Cluster` が `RemoteConnectionProbe=True` / `Available=True` になる
    - 抽出した kubeconfig で `kubectl get nodes` が成功する（NoWorkers 可）
...
- [x] [P1] Treat kubeconfig source Secret `data["value"]` 欠落を非致命にする: hard error ではなく Condition/Event + `RequeueAfter` で待機/復帰できるようにする。Touch: `internal/controller/kany8scontrolplane_controller.go`, `internal/controller/kany8scontrolplane_kubeconfig_test.go`。DoD: data key が後から追加されたら target Secret が追従し、ログスパムにならない。Run: `make test`。
- [x] [P2] Keep `KubeconfigSecretReconciled` Condition fresh while waiting: source Secret NotFound 時に Condition を `False` (Waiting) にして stale を避ける（event は変化時のみ）。Touch: `internal/controller/kany8scontrolplane_controller.go`, `internal/controller/kany8scontrolplane_kubeconfig_test.go`。DoD: 以前 True だった Condition が NotFound 待機中に True のまま残らない。Run: `make test`。
- [x] [P2] Decide readiness semantics for kubeconfig: `Ready=True` が endpoint のみで良いか / kubeconfig secret reconcile も含めるかを明文化し、Conditions/テストを揃える。Touch: `docs/rgd-contract.md`（or `docs/design.md`）, `internal/controller/kany8scontrolplane_controller.go`, tests。DoD: Ready/Conditions の意味が一貫する。Run: `make test`。
...
- [x] [P1] (Issue #2) dynamic watch のイベント喪失を無言で起こさない: channel full 時の drop をログ/メトリクス化し、必要なら coalescing/バッファ見直し/バックプレッシャを導入する。Touch: `internal/dynamicwatch/watcher.go`（+ 連携箇所）。DoD: drop が観測でき、通常負荷で status 変化を取り逃がさない。Run: `make test`.
- [x] [P1] (Issue #10) `InstanceWatcher.EnsureWatch` 失敗を握りつぶさない: watch が壊れても "Ready 後に二度と reconcile されない" 事態を避ける（watch setup の retry / fallback periodic requeue など）。Touch: `internal/controller/kany8scontrolplane_controller.go`。DoD: watch 不能でも一定時間内に追従が再開する。Run: `make test`.
