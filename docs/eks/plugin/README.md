...
- AWS 資格情報は ACK controller と同じ方式: `aws.credentials.secretName` を指定すると shared credentials file の Secret を mount し、空のままなら SDK default chain（IRSA / EKS Pod Identity / EC2 IMDS）にフォールバックする。IRSA の role ARN は `serviceAccount.annotations."eks.amazonaws.com/role-arn"` に書く。
