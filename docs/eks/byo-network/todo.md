...
- [x] Define ClusterClass variables in `docs/eks/byo-network/manifests/clusterclass-eks-byo.yaml` as the single source of truth: `region`, `eks-version`, `vpc-control-plane-subnet-ids` (minItems=2; control plane ENI placement), `vpc-node-subnet-ids` (minItems=2; private + NAT for Fargate/NodePool), `vpc-security-group-ids` (allow empty list), `eks-public-access-cidrs` (minItems=1; explicit).
