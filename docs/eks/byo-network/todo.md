...
- [x] Define ClusterClass variables in `docs/eks/byo-network/manifests/clusterclass-eks-byo.yaml` as the single source of truth: `region`, `eks-version`, `vpc-control-plane-subnet-ids` (minItems=2; control plane ENI placement), `vpc-node-subnet-ids` (minItems=1; private subnets for Fargate/NodePool; egress via NAT or VPC endpoints), `vpc-security-group-ids` (allow empty list), `eks-public-access-cidrs` (minItems=1; explicit).
