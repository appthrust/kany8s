...
- `vpc.endpointPrivateAccess` (任意; default=true)
- `vpc.endpointPublicAccess` (任意; default=true)
- `publicAccessCIDRs` (必須: 明示させる。"0.0.0.0/0" でもよいが意図的に指定する)
- `access.authenticationMode` (任意; default=`API_AND_CONFIG_MAP`)
...
- `eks-endpoint-private-access` (optional; default=true)
- `eks-endpoint-public-access` (optional; default=true)
...
- `region` / `vpc-control-plane-subnet-ids` / `vpc-node-subnet-ids` / `vpc-security-group-ids` / `eks-public-access-cidrs` を variables で指定
