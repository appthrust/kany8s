...
# - NODE_SUBNET_ID_*: karpenter Fargate + 既定 EC2NodeClass 用。private subnet が必須。image pull のための egress は NAT default route または VPC endpoints (ecr.api, ecr.dkr, s3, sts, logs) のいずれかで確保する (controller は NAT 未設定を warning として報告)。AWS API としては 1 subnet で動くが、HA のため >=2 AZ 推奨。
...
- `vpc-node-subnet-ids` <- `NODE_SUBNET_ID_1` (+ optional `NODE_SUBNET_ID_2`)
  - karpenter Fargate profile + default EC2NodeClass `subnetSelectorTerms`; must be private subnets. image pull 用の egress は NAT default route か VPC endpoints (ecr.api, ecr.dkr, s3, sts, logs) のいずれかで確保する (controller は NAT 未設定を warning として報告し、VPC endpoints で代替可能)。`>=1` subnet 必須 (AWS FargateProfile は 1 subnet を許容)。HA のため `>=2` AZ 推奨 (AZ 数 < 2 で controller が warning event を発火するが reconcile はブロックしない)
