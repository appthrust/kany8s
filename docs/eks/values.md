...
aws eks describe-addon-versions --region "$AWS_REGION" --addon-name kube-proxy \
  --query 'addons[0].compatibilities[].clusterVersion' --output text
```
...
反映方法は `docs/eks/manifests/cluster.yaml.tpl` / `docs/eks/manifests/controlplane-only.yaml.tpl` の
`publicAccessCIDRs` コメントを外して値を入れます。
