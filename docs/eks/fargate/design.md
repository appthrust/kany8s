...
従って、この設計で指定する `vpc-node-subnet-ids` (FargateProfile + NodePool 向け) は原則 private subnet を想定する (egress は NAT または VPC endpoints のいずれかで確保する; controller は NAT default route 未設定を warning として報告し、VPC endpoints で代替可能)。`vpc-control-plane-subnet-ids` (EKS control plane ENI 向け) は NAT egress 不要で、endpoint アクセスモードに応じて public/private/isolated を選ぶ。
