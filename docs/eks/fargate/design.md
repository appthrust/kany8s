...
従って、この設計で指定する `vpc-node-subnet-ids` (FargateProfile + NodePool 向け) は原則 private subnet (NAT egress 必須) を想定する。`vpc-control-plane-subnet-ids` (EKS control plane ENI 向け) は NAT egress 不要で、endpoint アクセスモードに応じて public/private/isolated を選ぶ。
