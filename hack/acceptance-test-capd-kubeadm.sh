...
WORKLOAD_KUBERNETES_VERSION="${WORKLOAD_KUBERNETES_VERSION:-v1.34.0}"
WORKLOAD_NODE_IMAGE="${WORKLOAD_NODE_IMAGE:-kindest/node:v1.34.0}"
...
SELF_MANAGED_SAMPLE_TEMPLATE="${SELF_MANAGED_SAMPLE_TEMPLATE:-${repo_root}/test/acceptance_test/manifests/self-managed-docker/cluster.yaml.tpl}"
...
rendered_self_managed_manifest="${ARTIFACTS_DIR}/self-managed-cluster.yaml"
sed \
	-e "s/__CLUSTER_NAME__/${CLUSTER_NAME}/g" \
	-e "s/__NAMESPACE__/${NAMESPACE}/g" \
	-e "s/__KUBERNETES_VERSION__/${WORKLOAD_KUBERNETES_VERSION}/g" \
	-e "s|__NODE_IMAGE__|${WORKLOAD_NODE_IMAGE}|g" \
	"${SELF_MANAGED_SAMPLE_TEMPLATE}" >"${rendered_self_managed_manifest}"

apply_manifest_with_webhook_retry "${rendered_self_managed_manifest}"
