...
func TestKany8sKubeadmControlPlaneControllerScaffoldExists(t *testing.T) {
	root := findRepoRoot(t)

	controllerPath := filepath.Join(root, "internal", "controller", "controlplane", "kany8skubeadmcontrolplane_controller.go")
	controllerBytes, err := os.ReadFile(controllerPath)
	if err != nil {
		t.Fatalf("read %q: %v", controllerPath, err)
	}
	...
	wantControllerSubstrings := []string{
		...
		"func (r *Kany8sKubeadmControlPlaneReconciler) Reconcile",
	...
	mainGo := string(mainBytes)
	wantMainSubstrings := []string{
		"Kany8sKubeadmControlPlaneReconciler",
		"SetupWithManager(mgr)",
	}
	for _, want := range wantMainSubstrings {
		if !strings.Contains(mainGo, want) {
