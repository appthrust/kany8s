...
func TestKany8sClusterReconciler_KroMode_RequeuesWhenResourceGraphDefinitionNotFound(t *testing.T) {
	t.Parallel()
	...
	if res.RequeueAfter == 0 {
		t.Fatalf("expected RequeueAfter to be set while waiting for RGD")
	}
	...
	if cond.Reason != "ResourceGraphDefinitionNotFound" {
		t.Fatalf("Ready condition reason = %q, want %q", cond.Reason, "ResourceGraphDefinitionNotFound")
	}
	if !strings.Contains(cond.Message, testRGDName) {
		t.Fatalf("Ready condition message = %q, want to contain %q", cond.Message, testRGDName)
	}
	if got.Status.FailureReason != nil {
		t.Fatalf("status.failureReason = %q, want nil", *got.Status.FailureReason)
	}
...
func TestKany8sClusterReconciler_KroMode_RequeuesWhenResourceGraphDefinitionIsInvalid(t *testing.T) {
	t.Parallel()
...
func TestKany8sClusterReconciler_KroMode_ProceedsWhenOwnerClusterResolvedAndRGDDeclaresClusterUID(t *testing.T) {
	...
	if err := c.Get(ctx, client.ObjectKey{Name: testClusterName, Namespace: testNamespace}, gotInstance); err != nil {
		t.Fatalf("get kro instance: %v", err)
	}

	clusterUID, found, err := unstructured.NestedString(gotInstance.Object, "spec", "clusterUID")
	if err != nil {
		t.Fatalf("get spec.clusterUID: %v", err)
	}
	if !found {
		t.Fatalf("expected spec.clusterUID to be set")
	}
	if clusterUID != string(owner.UID) {
		t.Fatalf("spec.clusterUID = %q, want %q", clusterUID, owner.UID)
	}
}

func TestKany8sClusterReconciler_KroMode_DoesNotInjectClusterUIDWhenRGDDoesNotDeclareIt(t *testing.T) {
	t.Parallel()
	...
	c := fake.NewClientBuilder().WithScheme(scheme).WithObjects(kc, rgd, owner).WithStatusSubresource(kc).Build()
	r := &Kany8sClusterReconciler{Client: c, Scheme: scheme}
	...
	_, found, err := unstructured.NestedString(gotInstance.Object, "spec", "clusterUID")
	if err != nil {
		t.Fatalf("get spec.clusterUID: %v", err)
	}
	if found {
		t.Fatalf("expected spec.clusterUID to be unset")
	}
}
